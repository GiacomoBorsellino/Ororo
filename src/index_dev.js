...
const responseO = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${apiMeteo}`)
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function(position) {
                    ...
                    const response = await fetch(`https://api.waqi.info/feed/geo:${lon};${lat}/?token=${apiKey}`)
                    const result = await response.json()
                ...
                try {
                    ...
                    // Chiamata API OpenWeather
                    const responseO = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiMeteo}`)
                    const resultO = await responseO.json()
                    try {
                        document.body.getElementsByClassName("city")[0].innerHTML = resultO.name
